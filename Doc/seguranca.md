...
- **Ajuste Necessário:** Implementar _Hashing_ de senhas utilizando bibliotecas como `passlib` com algoritmo `bcrypt` ou `Argon2`. **Nunca** salvar a senha original.
...
- **Risco:** Ataques de _Cross-Site Request Forgery_ (CSRF).
...
| Ordem | Tarefa                                      | Arquivos Impactados                | Status          | Última Revisão |
| :---- | :------------------------------------------ | :--------------------------------- | :-------------- | :------------- |
| 1     | Instalar `passlib[bcrypt]` e hash de senhas | `backend/services/auth_service.py` | 🟢 Corrigido    | 18/02/2025     |
| 2     | Implementar JWT e Proteção de Rotas         | `backend/dependencies/auth.py`     | 🟢 Corrigido    | 18/02/2025     |
| 3     | Centralizar Configurações no `config.py`    | `backend/config.py`                | 🟢 Corrigido    | 18/02/2025     |
| 4     | Sanitizar o `docker-compose.yml`            | `docker-compose.yml`, `.env`       | 🟡 Em progresso | 18/02/2025     |
| 5     | Adicionar validação de e-mail (Regex)       | `backend/schemas/ai_schemas.py`    | 🔴 Pendente     | 18/02/2025     |
